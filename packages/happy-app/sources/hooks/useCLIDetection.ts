        ...
        const detectCLIs = async () => {
            ...
            try {
                ...
                if (result.success && result.exitCode === 0) {
                    ...
                    const cliStatus: Partial<Record<'claude' | 'codex' | 'gemini' | 'opencode', boolean>> = {};

                    lines.forEach(line => {
                        const [cli, status] = line.split(':');
                        if (cli && status) {
                            const cliName = cli.trim();
                            if (cliName === 'claude' || cliName === 'codex' || cliName === 'gemini' || cliName === 'opencode') {
                                cliStatus[cliName] = status.trim() === 'true';
                            }
                        }
                    });
