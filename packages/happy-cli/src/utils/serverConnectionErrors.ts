    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `hellovibe auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `hellovibe auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  HelloVibe server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
