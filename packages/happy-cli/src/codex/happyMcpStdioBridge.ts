 ...
 *
 * Configure the target HTTP MCP URL via env var `HELLOVIBE_HTTP_MCP_URL` or
...
async function main() {
  ...
  const baseUrl = urlFromArgs || process.env.HELLOVIBE_HTTP_MCP_URL || process.env.HAPPY_HTTP_MCP_URL || '';

  if (!baseUrl) {
    // Write to stderr; never stdout.
    process.stderr.write(
      '[hellovibe-mcp] Missing target URL. Set HELLOVIBE_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
  ...
  async function ensureHttpClient(): Promise<Client> {
    ...
    const client = new Client(
      { name: 'hellovibe-stdio-bridge', version: '1.0.0' },
      { capabilities: {} }
    );
...
main().catch((err) => {
  try {
    process.stderr.write(`[hellovibe-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  } finally {
    process.exit(1);
  }
});
